/* cannot resolve 3.2*2.2, then back 2 and . and 2 then press 0000 shows 3.2*0000, should show 3.2*0;
Exponentiation xy is very similar to percent button code
*/
function count() {
...
if(arr.indexOf('.')!==-1){
   ...
   return sub();
    ...
    if(flag){$result.text('');flag=false;arr=[];return check();}
...
if(str===''){return $result.text('')}
   ...
   else{  var equa=eval($result.text());
       console.log(equa)
...
if(str===''){return $result.text('')}
else{var neg=fixNumber(eval($result.text()));
arr=[];
arr.push(neg*-1);
...
if(str===''){return $result.text('')}
  else{  var squa=fixNumber(eval($result.text()));
...
if(str===''){return $result.text('')}
   else{ var sqt=fixNumber(Math.sqrt(eval($result.text())));
    $result.text(sqt);
   arr=[];
arr.push(sqt);
  return sub(); } 
